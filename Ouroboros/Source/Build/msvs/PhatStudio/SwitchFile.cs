...
namespace PhatStudio
{
	static class SwitchFile
	{
		...
		/// <summary>
		/// Iterate recursively over all project items looking for a match
		/// 
		/// First, get the list of possible file endings
		/// Then, look in same folder
		/// Then, look in project folder and use the first match
		/// </summary>
		public static ProjectItem FindProjectItemByName(ProjectItem projectItem, string name)
		{
			foreach (ProjectItem subProjectItem in projectItem.ProjectItems)
			{
				ProjectItem found = FindProjectItemByName(subProjectItem, name);
				if (found != null)
					return found;
				if (subProjectItem.Name == name)
