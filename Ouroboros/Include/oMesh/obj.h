...
struct info_t
{
	...
	// reduction/duplication is done internally). If a channel is nullptr
	// then that data is not available.
	...
	path_t obj_path;
	path_t mtl_path;
	const group_t* groups; // the size of this array is the same as mesh::info_t::num_subsets
...
struct init_t
{
	...
	uint32_t est_num_vertices;
	uint32_t est_num_indices;
